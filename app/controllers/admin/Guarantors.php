...
class Guarantors extends MY_Controller
{
    public function __construct()
    {
        ...
        if ($this->Customer || $this->Supplier || $this->Guarantor) {
            $this->session->set_flashdata('warning', lang('access_denied'));
            redirect($_SERVER['HTTP_REFERER']);
        }
        $this->lang->admin_load('guarantors', $this->Settings->user_language);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run('companies/add') == true) {
            ...
            admin_redirect('guarantors');
        }

        if ($this->form_validation->run() == true && $sid = $this->companies_model->addCompany($data)) {
            $this->session->set_flashdata('message', $this->lang->line('guarantor_added'));
            $ref = isset($_SERVER['HTTP_REFERER']) ? explode('?', $_SERVER['HTTP_REFERER']) : null;
            admin_redirect($ref[0] . '?guarantor=' . $sid);
        } else {
            $this->data['error']    = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'guarantors/add', $this->data);
        }
    }
    ...
    public function add_user($company_id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data, $active, $notify)) {
            ...
            $this->load->view($this->theme . 'guarantors/add_user', $this->data);
        }
    }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->companies_model->deleteguarantor($id)) {
            $this->sma->send_json(['error' => 0, 'msg' => lang('guarantor_deleted')]);
        } else {
            $this->sma->send_json(['error' => 1, 'msg' => lang('guarantor_x_deleted_have_purchases')]);
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->companies_model->updateCompany($id, $data)) {
            $this->session->set_flashdata('message', $this->lang->line('guarantor_updated'));
            redirect($_SERVER['HTTP_REFERER']);
        } else {
            $this->data['guarantor'] = $company_details;
            $this->data['error']    = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'guarantors/edit', $this->data);
        }
    }

    public function getGuarantor($id = null)
    {
    ...
    public function getGuarantors()
    {
        ...
        $this->datatables
            ...
            ->from('companies')
            ->where('group_name', 'guarantor')
            ->add_column('Actions', "<div class=\"text-center\"><a class=\"tip\" title='" . $this->lang->line('list_products') . "' href='" . admin_url('products?guarantor=$1') . "'><i class=\"fa fa-list\"></i></a> <a class=\"tip\" title='" . $this->lang->line('list_users') . "' href='" . admin_url('guarantors/users/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-users\"></i></a> <a class=\"tip\" title='" . $this->lang->line('add_user') . "' href='" . admin_url('guarantors/add_user/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-plus-circle\"></i></a> <a class=\"tip\" title='" . $this->lang->line('edit_guarantor') . "' href='" . admin_url('guarantors/edit/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . $this->lang->line('delete_guarantor') . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('guarantors/delete/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", 'id');
        //->unset_column('id');
    ...
    public function import_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES['csv_file'])) {
                ...
                foreach ($arrResult as $key => $value) {
                    ...
                    if (empty($guarantor['company']) || empty($guarantor['name']) || empty($guarantor['email'])) {
                        $this->session->set_flashdata('error', lang('company') . ', ' . lang('name') . ', ' . lang('email') . ' ' . lang('are_required') . ' (' . lang('line_no') . ' ' . $rw . ')');
                        admin_redirect('guarantors');
                    } else {
                        if ($this->Settings->indian_gst && empty($guarantor['state'])) {
                            $this->session->set_flashdata('error', lang('state') . ' ' . lang('is_required') . ' (' . lang('line_no') . ' ' . $rw . ')');
                            admin_redirect('guarantors');
                        }
                        if ($guarantor_details = $this->companies_model->getCompanyByEmail($guarantor['email'])) {
                            if ($guarantor_details->group_id == 7) {
                                $updated .= '<p>' . lang('guarantor_updated') . ' (' . $guarantor['email'] . ')</p>';
                                $this->companies_model->updateCompany($guarantor_details->id, $guarantor);
                            }
                        } else {
                            $data[] = $guarantor;
                        }
        ...
        if ($this->form_validation->run() == true && !empty($data)) {
            if ($this->companies_model->addCompanies($data)) {
                $this->session->set_flashdata('message', $this->lang->line('guarantors_added') . $updated);
                admin_redirect('guarantors');
            }
        } else {
            if (isset($data) && empty($data)) {
                if ($updated) {
                    ...
                    $this->session->set_flashdata('warning', lang('data_x_guarantors'));
                }
                admin_redirect('guarantors');
            }
            ...
            $this->data['error']    = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'guarantors/import', $this->data);
        }
    }
    ...
    public function index($action = null)
    {
        ...
        $bc                   = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('guarantors')]];
        $meta                 = ['page_title' => lang('guarantors'), 'bc' => $bc];
        $this->page_construct('guarantors/index', $meta, $this->data);
    }
    ...
    public function suggestions($term = null, $limit = null)
    {
        ...
        $rows['results'] = $this->companies_model->getguarantorsuggestions($term, $limit);
        $this->sma->send_json($rows);
    }

    public function guarantor_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    if ($error) {
                        $this->session->set_flashdata('warning', lang('guarantors_x_deleted_have_purchases'));
                    } else {
                        $this->session->set_flashdata('message', $this->lang->line('guarantors_deleted'));
                    }
                    redirect($_SERVER['HTTP_REFERER']);
                }
                ...
                $this->session->set_flashdata('error', $this->lang->line('no_guarantor_selected'));
                redirect($_SERVER['HTTP_REFERER']);
            }
        } else {
            $this->session->set_flashdata('error', validation_errors());
    ...
    public function users($company_id = null)
    {
        ...
        $this->load->view($this->theme . 'guarantors/users', $this->data);
    }
    ...
    public function view($id = null)
    {
        ...
        $this->data['error']    = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['guarantor'] = $this->companies_model->getCompanyByID($id);
        $this->load->view($this->theme . 'guarantors/view', $this->data);
    }
}
