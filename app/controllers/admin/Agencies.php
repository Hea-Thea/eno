...
class Agencies extends MY_Controller
{
    public function __construct()
    {
        ...
        if ($this->Customer || $this->Supplier || $this->Agency) {
            $this->session->set_flashdata('warning', lang('access_denied'));
            redirect($_SERVER['HTTP_REFERER']);
        }
        $this->lang->admin_load('agencies', $this->Settings->user_language);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run('companies/add') == true) {
            ...
            admin_redirect('agencies');
        }

        if ($this->form_validation->run() == true && $sid = $this->companies_model->addCompany($data)) {
            $this->session->set_flashdata('message', $this->lang->line('agency_added'));
            $ref = isset($_SERVER['HTTP_REFERER']) ? explode('?', $_SERVER['HTTP_REFERER']) : null;
            admin_redirect($ref[0] . '?agency=' . $sid);
        } else {
            $this->data['error']    = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'agencies/add', $this->data);
        }
    }
    ...
    public function add_user($company_id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email, $additional_data, $active, $notify)) {
            ...
            $this->load->view($this->theme . 'agencies/add_user', $this->data);
        }
    }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->companies_model->deleteagency($id)) {
            $this->sma->send_json(['error' => 0, 'msg' => lang('agency_deleted')]);
        } else {
            $this->sma->send_json(['error' => 1, 'msg' => lang('agency_x_deleted_have_purchases')]);
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->companies_model->updateCompany($id, $data)) {
            $this->session->set_flashdata('message', $this->lang->line('agency_updated'));
            redirect($_SERVER['HTTP_REFERER']);
        } else {
            $this->data['agency'] = $company_details;
            $this->data['error']    = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'agencies/edit', $this->data);
        }
    }
    ...
    public function getAgencies()
    {
        ...
        $this->datatables
            ...
            ->from('companies')
            ->where('group_name', 'agency')
            ->add_column('Actions', "<div class=\"text-center\"><a class=\"tip\" title='" . $this->lang->line('list_products') . "' href='" . admin_url('products?agency=$1') . "'><i class=\"fa fa-list\"></i></a> <a class=\"tip\" title='" . $this->lang->line('list_users') . "' href='" . admin_url('agencies/users/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-users\"></i></a> <a class=\"tip\" title='" . $this->lang->line('add_user') . "' href='" . admin_url('agencies/add_user/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-plus-circle\"></i></a> <a class=\"tip\" title='" . $this->lang->line('edit_agency') . "' href='" . admin_url('agencies/edit/$1') . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . $this->lang->line('delete_agency') . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('agencies/delete/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", 'id');
        //->unset_column('id');
    ...
    public function import_csv()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES['csv_file'])) {
                ...
                foreach ($arrResult as $key => $value) {
                    ...
                    if (empty($agency['company']) || empty($agency['name']) || empty($agency['email'])) {
                        $this->session->set_flashdata('error', lang('company') . ', ' . lang('name') . ', ' . lang('email') . ' ' . lang('are_required') . ' (' . lang('line_no') . ' ' . $rw . ')');
                        admin_redirect('agencies');
                    } else {
                        if ($this->Settings->indian_gst && empty($agency['state'])) {
                            $this->session->set_flashdata('error', lang('state') . ' ' . lang('is_required') . ' (' . lang('line_no') . ' ' . $rw . ')');
                            admin_redirect('agencies');
                        }
                        if ($agency_details = $this->companies_model->getCompanyByEmail($agency['email'])) {
                            if ($agency_details->group_id == 6) {
                                $updated .= '<p>' . lang('agency_updated') . ' (' . $agency['email'] . ')</p>';
                                $this->companies_model->updateCompany($agency_details->id, $agency);
                            }
                        } else {
                            $data[] = $agency;
                        }
        ...
        if ($this->form_validation->run() == true && !empty($data)) {
            if ($this->companies_model->addCompanies($data)) {
                $this->session->set_flashdata('message', $this->lang->line('agencies_added') . $updated);
                admin_redirect('agencies');
            }
        } else {
            if (isset($data) && empty($data)) {
                if ($updated) {
                    ...
                    $this->session->set_flashdata('warning', lang('data_x_agencies'));
                }
                admin_redirect('agencies');
            }
            ...
            $this->data['error']    = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'agencies/import', $this->data);
        }
    }
    ...
    public function index($action = null)
    {
        ...
        $bc                   = [['link' => base_url(), 'page' => lang('home')], ['link' => '#', 'page' => lang('agencies')]];
        $meta                 = ['page_title' => lang('agencies'), 'bc' => $bc];
        $this->page_construct('agencies/index', $meta, $this->data);
    }
    ...
    public function suggestions($term = null, $limit = null)
    {
        ...
        $rows['results'] = $this->companies_model->getagenciesuggestions($term, $limit);
        $this->sma->send_json($rows);
    }

    public function agency_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    if ($error) {
                        $this->session->set_flashdata('warning', lang('agencies_x_deleted_have_purchases'));
                    } else {
                        $this->session->set_flashdata('message', $this->lang->line('agencies_deleted'));
                    }
                    redirect($_SERVER['HTTP_REFERER']);
                }
                ...
                $this->session->set_flashdata('error', $this->lang->line('no_agency_selected'));
                redirect($_SERVER['HTTP_REFERER']);
            }
        } else {
            $this->session->set_flashdata('error', validation_errors());
    ...
    public function users($company_id = null)
    {
        ...
        $this->load->view($this->theme . 'agencies/users', $this->data);
    }
    ...
    public function view($id = null)
    {
        ...
        $this->data['error']    = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['agency'] = $this->companies_model->getCompanyByID($id);
        $this->load->view($this->theme . 'agencies/view', $this->data);
    }
}
