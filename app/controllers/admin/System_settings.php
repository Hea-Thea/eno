...
class system_settings extends MY_Controller
{
    ...
    {
        $this->form_validation->set_rules('name', lang('loan_type_name'), 'trim|required|is_unique[loan_types.name]|alpha_numeric_spaces');
        $this->form_validation->set_rules('description', lang('description'), 'trim|required');
        ...
        } elseif ($this->input->post('add_loan_type')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('system_settings/loan_types');
        }

        if ($this->form_validation->run() == true && $this->settings_model->addLoanType($data)) {
            $this->session->set_flashdata('message', lang('loan_type_added'));
            admin_redirect('system_settings/loan_types');
        } else {
            $this->data['error']    = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_loan_type', $this->data);
        }
    }

    public function edit_loan_type($id = null)
    {
        $this->form_validation->set_rules('name', lang('loan_type_name'), 'trim|required|alpha_numeric_spaces');
        $loan_type_details = $this->site->getLoanTypeByID($id);
        if ($this->input->post('name') != $loan_type_details->name) {
            $this->form_validation->set_rules('name', lang('loan_type_name'), 'required|is_unique[loan_types.name]');
        }
        ...
        } elseif ($this->input->post('edit_loan_type')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('system_settings/loan_types');
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateLoanType($id, $data)) {
            $this->session->set_flashdata('message', lang('loan_type_updated'));
            admin_redirect('system_settings/loan_types');
        } else {
            $this->data['error']    = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['loan_type']    = $loan_type_details;
            $this->load->view($this->theme . 'settings/edit_loan_type', $this->data);
        }
    }
    ...
    public function delete_loan_type($id = null)
    {
        ...
        if ($this->settings_model->loanTypeHasLoans($id)) {
            $this->sma->send_json(['error' => 1, 'msg' => lang('loan_type_has_loans')]);
        }

        if ($this->settings_model->deleteLoanType($id)) {
            $this->sma->send_json(['error' => 0, 'msg' => lang('loan_type_deleted')]);
        }
    }

    public function loan_type_actions()
    {
        $this->form_validation->set_rules('form_action', lang('form_action'), 'required');
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    foreach ($_POST['val'] as $id) {
                        $this->settings_model->deleteLoanType($id);
                    }
                    $this->session->set_flashdata('message', lang('loan_types_deleted'));
                    redirect($_SERVER['HTTP_REFERER']);
                }
                ...
                if ($this->input->post('form_action') == 'export_excel') {
                    $this->load->library('excel');
                    $this->excel->setActiveSheetIndex(0);
                    $this->excel->getActiveSheet()->setTitle(lang('loan_types'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('name'));
                    $this->excel->getActiveSheet()->SetCellValue('B1', lang('code'));
                    ...
                    $row = 2;
                    foreach ($_POST['val'] as $id) {
                        $loan_type = $this->site->getLoanTypeByID($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $loan_type->name);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $loan_type->code);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $loan_type->description);
                        $row++;
                    }

                    $this->excel->getActiveSheet()->getColumnDimension('A')->setWidth(20);
                    $this->excel->getDefaultStyle()->getAlignment()->setVertical('center');
                    $filename = 'loan_types_' . date('Y_m_d_H_i_s');
                    $this->load->helper('excel');
                    create_excel($this->excel, $filename);
                }
            } else {
    ...
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc                  = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings'), 'page' => lang('system_settings')], ['link' => '#', 'page' => lang('loan_types')]];
        $meta                = ['page_title' => lang('loan_types'), 'bc' => $bc];
        $this->page_construct('settings/loan_types', $meta, $this->data);
    }
    ...
    public function getLoanTypes()
    {
        $this->load->library('datatables');
        $this->datatables
            ->select('loan_type_id as id, code, name, description')
            ->from('loan_types')
            ->add_column('Actions', "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_loan_type/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='" . lang('edit_loan_type') . "'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang('delete_loan_type') . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_loan_type/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", 'id');
    ...
    {
        $this->form_validation->set_rules('name', lang('holiday_name'), 'trim|required|is_unique[loan_holidays.name]|alpha_numeric_spaces');
        $this->form_validation->set_rules('description', lang('description'), 'trim|required');

        if ($this->form_validation->run() == true) {
            $data = [
                'name'        => $this->input->post('name'),
                'from_date'   => $this->sma->fsd(trim($this->input->post('from_date'))),
                'to_date'   => $this->sma->fsd(trim($this->input->post('to_date'))),
                'description' => $this->input->post('description'),
            ];

        } elseif ($this->input->post('add_holiday')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('system_settings/holidays');
        }

        if ($this->form_validation->run() == true && $this->settings_model->addHoliday($data)) {
    ...
    {
        $this->form_validation->set_rules('name', lang('holiday_name'), 'trim|required|alpha_numeric_spaces');
        $holiday_details = $this->site->getHolidayByID($id);
        if ($this->input->post('name') != $holiday_details->name) {
            $this->form_validation->set_rules('name', lang('holiday_name'), 'required|is_unique[loan_holidays.name]');
        }
        ...
        } elseif ($this->input->post('edit_holiday')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('system_settings/edit_holiday');
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateHoliday($id, $data)) {
            $this->session->set_flashdata('message', lang('holiday_updated'));
            admin_redirect('system_settings/holidays');
        } else {
            $this->data['error']    = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['holiday']    = $holiday_details;
    ...
    public function holiday_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    foreach ($_POST['val'] as $id) {
                        $this->settings_model->deleteHoliday($id);
                    }
                ...
                if ($this->input->post('form_action') == 'export_excel') {
                    $this->load->library('excel');
                    $this->excel->setActiveSheetIndex(0);
                    $this->excel->getActiveSheet()->setTitle(lang('holidays'));
                    $this->excel->getActiveSheet()->SetCellValue('A1', lang('name'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        $loan_type = $this->site->getLoanTypeByID($id);
                        $this->excel->getActiveSheet()->SetCellValue('A' . $row, $loan_type->name);
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $loan_type->from_date);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $loan_type->to_date);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $loan_type->description);
                        $row++;
                    }

                    $this->excel->getActiveSheet()->getColumnDimension('A')->setWidth(20);
                    $this->excel->getDefaultStyle()->getAlignment()->setVertical('center');
                    $filename = 'holidays_' . date('Y_m_d_H_i_s');
                    $this->load->helper('excel');
                    create_excel($this->excel, $filename);
                }
            } else {
    ...
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc                  = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings'), 'page' => lang('system_settings')], ['link' => '#', 'page' => lang('holidays')]];
    ...
    public function getHolidays()
    {
        ...
        $this->datatables
            ...
            ->from('loan_holidays')
            ->add_column('Actions', "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_holiday/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='" . lang('edit_holiday') . "'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang('delete_holiday') . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_holiday/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", 'id');
