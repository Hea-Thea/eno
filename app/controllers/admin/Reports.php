...
class Reports extends MY_Controller
{
    ...
    public function getLoanPaymentsReport($pdf = null, $xls = null)
    {
        $this->sma->checkPermissions('payments', true);
        ...
        $payment_ref    = $this->input->get('payment_ref') ? $this->input->get('payment_ref') : null;
        $paid_by        = $this->input->get('paid_by') ? $this->input->get('paid_by') : null;
        $loan_ref       = $this->input->get('loan_ref') ? $this->input->get('loan_ref') : null;
        ...
        if ($pdf || $xls) {
            $this->db
                ->select('' . $this->db->dbprefix('payments') . '.date, ' . $this->db->dbprefix('payments') . '.reference_no as payment_ref, ' . $this->db->dbprefix('loans') . '.reference_no as loan_ref,paid_by, payments.amount, type')
                ->from('payments')
            ...
            if ($customer) {
                $this->db->where('loans.customer_id', $customer);
            }
            ...
            if ($loan_ref) {
                $this->db->like('loans.reference_no', $loan_ref, 'both');
            }
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('A1', lang('date'));
                $this->excel->getActiveSheet()->SetCellValue('B1', lang('payment_reference'));
                $this->excel->getActiveSheet()->SetCellValue('C1', lang('loan_reference'));
                ...
                foreach ($data as $data_row) {
                    $this->excel->getActiveSheet()->SetCellValue('A' . $row, $this->sma->hrld($data_row->date));
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, $data_row->payment_ref);
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, $data_row->loan_ref);
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->type);
                    if ($data_row->type == 'returned' || $data_row->type == 'sent') {
                        $total -= $data_row->amount;
                    } else {
                        $total += $data_row->amount;
            ...
            $this->load->library('datatables');
            $this->datatables
                ->select("DATE_FORMAT({$this->db->dbprefix('payments')}.date, '%Y-%m-%d %T') as date, " . $this->db->dbprefix('payments') . '.reference_no as payment_ref, ' . $this->db->dbprefix('loans') . '.reference_no as loan_ref, ' . "paid_by, payments.amount, type, {$this->db->dbprefix('payments')}.id as id")
                ->from('payments')
                ->join('loans', 'payments.loan_id=loans.id', 'left')
                ->group_by('payments.id');
            ...
            if ($customer) {
                $this->datatables->where('loans.customer_id', $customer);
            }
            ...
            if ($loan_ref) {
                $this->datatables->like('loans.reference_no', $loan_ref, 'both');
            }
    ...
    public function loan_payments()
    {
        ...
        $bc                         = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('reports'), 'page' => lang('reports')], ['link' => '#', 'page' => lang('payments_report')]];
        $meta                       = ['page_title' => lang('payments_report'), 'bc' => $bc];
        $this->page_construct('reports/loan_payments', $meta, $this->data);
    }
