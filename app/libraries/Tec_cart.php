...
class Tec_cart
{
    ...
    protected function _save_cart_app($cart_id, $user_id)
    {
        ...
        if ($this->db->get_where('cart', ['id' => $cart_id])->num_rows() > 0) {
            return $this->db->update('cart', ['time' => time(), 'user_id' => $user_id, 'data' => json_encode($this->_cart_contents)], ['id' => $cart_id]);
        } else {
            return $this->db->insert('cart', ['id' => $cart_id, 'time' => time(), 'user_id' => $user_id, 'data' => json_encode($this->_cart_contents)]);
        }
    }
    ...
    public function destroyFromApp($cart_id)
    {
        $this->_empty();
        return $this->db->delete('cart', ['id' => $cart_id]);
    }
    ...
    public function removeFromApp($rowid, $cart_id, $user_id)
    {
        unset($this->_cart_contents[$rowid]);
        $this->_save_cart_app($cart_id, $user_id);
        return true;
    }
