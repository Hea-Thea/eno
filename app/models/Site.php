...
class Site extends CI_Model
{
    ...
    public function syncLoanPayments($id)
    {
        $loan = $this->getLoanByID($id);
        if ($payments = $this->getLoanPayments($id)) {
            $paid        = 0;
            $grand_total = $loan->grand_total + $loan->rounding;
            foreach ($payments as $payment) {
                $paid += $payment->amount;
            }

            $payment_status = $paid == 0 ? 'pending' : $loan->payment_status;
            if ($this->sma->formatDecimal($grand_total) == 0 || $this->sma->formatDecimal($grand_total) <= $this->sma->formatDecimal($paid)) {
                $payment_status = 'paid';
            } elseif ($loan->due_date <= date('Y-m-d') && !$loan->loan_id) {
                $payment_status = 'due';
            } elseif ($paid != 0) {
                $payment_status = 'partial';
            }

            if ($this->db->update('loans', ['paid' => $paid, 'payment_status' => $payment_status], ['id' => $id])) {
                return true;
            }
        } else {
            $payment_status = ($loan->due_date <= date('Y-m-d')) ? 'due' : 'pending';
            if ($this->db->update('loans', ['paid' => 0, 'payment_status' => $payment_status], ['id' => $id])) {
                return true;
            }
        }
