...
class Settings_model extends CI_Model
{
    ...
    {
        if ($this->db->insert('loan_types', $data)) {
            return true;
        }
        return false;
    }

    public function addLoanTypes($data)
    {
        if ($this->db->insert_batch('loan_types', $data)) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->db->update('loan_types', $data, ['loan_type_id' => $id])) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->db->delete('loan_types', ['loan_type_id' => $id])) {
            return true;
        }
        return false;
    }
    ...
    {
        $q = $this->db->get_where('loan_types', ['loan_type_id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function loanTypeHasLoans($loan_type_id)
    {
        $q = $this->db->get_where('loans', ['using_loan_type_id' => $loan_type_id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    {
        if ($this->db->insert('loan_holidays', $data)) {
            ...
            while (strtotime($start) < strtotime($endDate)){
                ...
                $this->db->insert('loan_holiday_detail',$data_detail); 
                $i++;
            }
    ...
    {
        if ($this->db->insert_batch('loan_holidays', $data)) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->db->update('loan_holidays', $data, ['id' => $id])) {
            $this->db->delete('loan_holiday_detail',array('holiday_id'=>$id));
    ...
    {
        if ($this->db->delete('loan_holidays', ['id' => $id])) {
            $this->db->delete('loan_holiday_detail',array('holiday_id'=>$id));
            return true;
        }
        return false;
    ...
    {
        $q = $this->db->get_where('loan_holidays', ['id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
